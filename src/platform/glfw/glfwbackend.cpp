...
void GLFWBackend::ErrorCallback(int error, const char* description)
{
    fprintf(stderr, "Glfw Error %d: %s\n", error, description);
}

void GLFWBackend::SizeCallback(GLFWwindow* window, int width, int height)
{
    GLFWBackend* instance = reinterpret_cast<GLFWBackend*>(glfwGetWindowUserPointer(window));
    if (instance)
    {
...
GLFWBackend::GLFWBackend(int width, int height)
: _window(nullptr)
...
void GLFWBackend::Remove(const std::string& name)
{
    _child_windows.erase(name);
}

void GLFWBackend::Process()
{
    Initialize();
    if (_window == nullptr)
    {
...
void GLFWBackend::Initialize()
{
    ...
    // Setup window
    glfwSetErrorCallback(GLFWBackend::ErrorCallback);
