...
static void check_vk_result(VkResult err)
{
    if (err == 0)
        return;
    LOG_ERR("[vulkan] Error: VkResult = {}", static_cast<int>(err));
    if (err < 0)
        abort();
}
...
static VKAPI_ATTR VkBool32 VKAPI_CALL debug_report(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType, uint64_t object, size_t location, int32_t messageCode, const char* pLayerPrefix, const char* pMessage, void* pUserData)
{
    ...
    LOG_ERR("[vulkan] Debug report from ObjectType: {}, Message: {}", objectType, pMessage);
    return VK_FALSE;
}
...
void VulkanBackend::Initialize()
{
    ...
    // Setup window
    glfwSetErrorCallback(VulkanBackend::ErrorCallback);
    ...
    {
        LOG_ERR("GLFW: Vulkan Not Supported");
        return;
    }
    ...
    VkResult err = glfwCreateWindowSurface(g_Instance, _window, g_Allocator, &surface);
    check_vk_result(err);
